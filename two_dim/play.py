...
from two_dim.agents.her_cfg import TwoDimHERCfg
from two_dim.environment import TwoDimVecEnv
...
def main():
    ...
    if algo_name == "ppo":
        ...
        # asdict() flattened the nested TwoDimHERCfg into a plain dict; rehydrate it.
        her_cfg = cfg_dict.get("her_cfg")
        if isinstance(her_cfg, dict):
            cfg_dict["her_cfg"] = TwoDimHERCfg(**her_cfg)
        cfg = SACRunnerCfg(**cfg_dict)
    else:
        raise ValueError(f"Unknown algo_name in cfg: {algo_name}")

    env = TwoDimVecEnv(num_envs=1, max_episode_length=args.max_episode_length)
